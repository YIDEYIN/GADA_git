...
import numpy as np
from .model.utils import get_mixed_prediction, different_p_q_objectives, kl_coefficient


def train_test(train_loader, test_loader, diffusion, dae, dae_optimizer, cvae, cvae_optimizer, num_epoch, config):
    train_losses = []
    test_losses = []
    global_step = 0
    ...
    for epo in range(num_epoch):
        ...
        config.num_total_iter = n * num_epoch
        ...
        for i, (x_input, c_input, y_input) in enumerate(train_loader):
            ...
            cvae.load_state_dict(torch.load("cvae_independent_model"))
            ...
            # 使用 kl_anneal
            if config.cont_kl_anneal:
                kl_coeff = kl_coefficient(step=global_step,
                                          total_step=config.kl_anneal_portion_vada * config.num_total_iter,
                                          constant_step=config.kl_const_portion_vada * config.num_total_iter,
                                          min_kl_coeff=config.kl_const_coeff_vada,
                                          max_kl_coeff=config.kl_max_coeff_vada)
                # print('kl_coeff', kl_coeff)
            else:
                kl_coeff = 1.0

            # 计算cvae的elbo
            nelbo_loss = kl_coeff * kl_all + config.gamma * cvae_recon_loss_mean
            # 加上y的loss
...
def test_gada_joint(test_loader, diffusion, dae, cvae, epo, config):
    ...
    with torch.no_grad():
        for i, (x_input, c_input, y_input) in enumerate(test_loader):
            ...
            # 计算cvae的elbo
            nelbo_loss = kl_all + config.gamma * cvae_recon_loss_mean
            # 加上y的loss
            cvae_loss = nelbo_loss + cvae_mse_mean
            kl = cvae_neg_entropy + diffusion_cross_entropy
